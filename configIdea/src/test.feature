...
#####################################################################################
    When execute POST for @rest-client
    And set path '/v1/'
    And set path: '/v1/', url: 'http://testapi.com/'
    And set path '/v1/', url 'http://testapi.com/'
    And set path <path>, url <url>
    ...
    ##############################
  Scenario: User with 1 Managed Account = I with a NRG goal and Account is in completed State  and EED<Current Date

    Given User sent to PR MicroService with

      | interactionId | accountType | currentTxnState | currentExpectedExecutionDate |

      | NRG-002-test  | IRA         | txnCompleted    | Today-1                      |

    When It is processed by Portfolio Review MicroService

    Then Response should have Portfolio Advice along with

      | interactionId | errorDetails | UserResultCode | resultCode  | accountTransactionState |

      | NRG-002-test  | null         | TXN_CREATED    | TXN_CREATED | txnGen
