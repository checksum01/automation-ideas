Feature: Microservice
  ...
  Background:
    Given rest-client: rest-client1
    And   set url: 'https://reqres.in/'
    And   set path: 'v1/'

    Given rest-client: rest-client2
    And   set url: 'https://reqres.com/'
    And   set path: 'v2/'

    Given kafka: kafka

    Given def testvar = 'testVar'
    Given def myJson = read('/filepath/')

    Given rabidmq: @mainconfigservice
...
#####################################################################################
  ...
  Scenario Outline: Migrate country scenario with framework
    ...
    When execute POST for @rest-client1
    And set myJson.country = "UK"
    And set myJsonNOS.id = "44"

    When execute GET  for @rest-client2 ##### picks reqres attributes

    Then validate @rest-client1.request.country == @rest-client1.response.country
